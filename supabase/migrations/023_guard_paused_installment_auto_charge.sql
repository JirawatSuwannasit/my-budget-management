-- Extend the card-installment RPC so an explicitly paused debt cannot be
-- materialized by a stale/concurrent client request. Resuming does not backfill
-- paused periods; the next eligible card statement period starts from "now".
...
begin
  ...
  if not found or v_debt.active is false or v_debt.paused is true or v_debt.type <> 'installment'

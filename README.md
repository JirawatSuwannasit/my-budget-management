...
Quick Add lets each signed-in user save frequently used income, expense, or credit-card expense details in **Settings → Quick Add**. A template with an amount is a one-tap transaction; a template without an amount opens a compact, amount-focused Quick Fill control. Both modes use today's local date and the existing `create_finance_transaction` RPC, so balance, card, cycle, and validation behavior remains identical to the normal transaction form. Template references are ownership-checked and must remain active; a missing template account falls back only to the user's valid active default account.

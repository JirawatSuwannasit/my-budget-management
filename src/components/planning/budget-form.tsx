    ...
    <form action={formAction} className="grid gap-4 rounded-panel border border-slate-200 bg-white p-4 shadow-card">
      ...
      <div className={compact ? "grid gap-4" : "grid gap-4 sm:grid-cols-[1fr_0.75fr]"}>
        <label className="grid gap-2 text-sm font-black text-ink" htmlFor={"budget-label-" + idPrefix}>
          {t.form.budgetName}
          <input id={"budget-label-" + idPrefix} name="label" list="budget-examples" defaultValue={budget?.label ?? ""} placeholder={t.form.budgetPlaceholder} required className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-white" />
      ...
      <label className="grid gap-2 text-sm font-black text-ink" htmlFor={"budget-category-" + idPrefix}>
        ...
        <input id={"budget-category-" + idPrefix} name="category_name" list="budget-examples" defaultValue={budget?.category_name ?? budget?.label ?? ""} placeholder={t.form.matchingExpensePlaceholder} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-white" />
      </label>

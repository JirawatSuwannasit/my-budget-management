    ...
    <form action={formAction} className="grid gap-4 rounded-panel border border-line bg-surface p-4 shadow-card">
      ...
      <div className={compact ? "grid gap-4" : "grid gap-4 sm:grid-cols-[1fr_0.8fr]"}>
        <label className="grid gap-2 text-sm font-black text-ink" htmlFor={"annual-name-" + idPrefix}>
          ...
          <input id={"annual-name-" + idPrefix} name="name" list="annual-examples" defaultValue={annualExpense?.name ?? ""} placeholder={t.form.annualPlaceholder} required className="rounded-2xl border border-line bg-elevated px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-surface" />
        ...
        <label className="grid gap-2 text-sm font-black text-ink" htmlFor={"annual-category-" + idPrefix}>
          {t.form.category}
          <select id={"annual-category-" + idPrefix} name="category_name" defaultValue={annualExpense?.category_name ?? annualCategories[locale][0]} className="rounded-2xl border border-line bg-elevated px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-surface">
      ...
      <div className={compact ? "grid gap-4" : "grid gap-4 sm:grid-cols-2"}>
        <label className="grid gap-2 text-sm font-black text-ink">
          {t.form.annualAmount}
          <input name="annual_amount" type="number" min="0" step="0.01" defaultValue={annualExpense?.annual_amount ?? ""} placeholder="0.00" required className="rounded-2xl border border-line bg-elevated px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-surface" />
        </label>

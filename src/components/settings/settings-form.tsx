...
export function SettingsForm({ locale, currency, financialCycleStartDay, bonusMonths, defaultAccountId, accounts }: SettingsFormProps) {
  ...
  return (
    <form action={formAction} className="grid gap-4 rounded-panel border border-line bg-surface p-4 shadow-card md:p-5">
      <div className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-2 text-sm font-black text-ink">
          {t.language}
          <select name="locale" defaultValue={locale} className="rounded-2xl border border-line bg-elevated px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-surface">
        ...
        <label className="grid gap-2 text-sm font-black text-ink">
          ...
          <select name="currency" defaultValue={currency} disabled aria-readonly="true" className="rounded-2xl border border-line bg-elevated px-4 py-3 text-sm font-semibold text-muted outline-none transition">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-2 text-sm font-black text-ink">
          {t.cycleStartDay}
          <input name="financial_cycle_start_day" type="number" min="1" max="28" defaultValue={financialCycleStartDay} className="rounded-2xl border border-line bg-elevated px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-surface" />
        </label>

        <label className="grid gap-2 text-sm font-black text-ink">
          {t.defaultAccount}
          <select name="default_account_id" defaultValue={defaultAccountId ?? ""} className="rounded-2xl border border-line bg-elevated px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-surface">
      ...
      <fieldset className="grid gap-3">
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-6">
          {months.map((month) => (
            <label key={month.value} className="flex min-h-12 items-center gap-2 rounded-2xl border border-line bg-elevated px-3 py-2 text-sm font-bold text-muted">

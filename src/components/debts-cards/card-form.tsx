  ...
  return (
    <form action={formAction} className="grid gap-4 rounded-panel border border-slate-200 bg-white p-4 shadow-card">
      ...
      <label className="grid gap-2 text-sm font-black text-ink">
        {t.form.cardName}
        <input name="name" defaultValue={card?.name ?? ""} required placeholder={t.form.cardPlaceholder} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none transition focus:border-primary/60 focus:bg-white" />
      </label>

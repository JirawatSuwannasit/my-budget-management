...
import { deleteCreditCard } from "@/app/(private)/debts-cards/actions";
import { dictionaries, type Locale } from "@/lib/i18n/dictionaries";

export function DeleteCardForm({ id, locale }: { id: string; locale: Locale }) {
  ...
  return (
    ...
    >
      ...
      <button className="inline-flex min-h-11 items-center gap-2 rounded-full border border-danger/30 bg-danger/10 px-4 py-2 text-xs font-black text-danger transition hover:bg-danger/15">
        <Trash2 size={14} aria-hidden="true" /> {t.deleteCard}
      </button>
    </form>
  );
}

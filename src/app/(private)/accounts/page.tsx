      ...
      <section className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="grid gap-3">
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-muted shadow-card">{accounts.length} {t.countSuffix}</span>
          </div>

          {error ? <p className="rounded-panel border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-800">{t.loadError}: {error.message}</p> : null}
          {!error && accounts.length === 0 ? <p className="rounded-panel border border-dashed border-slate-300 bg-white/80 p-5 text-sm font-bold text-muted">{t.empty}</p> : null}
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={"rounded-full px-2.5 py-1 text-xs font-black " + (account.active ? "bg-emerald-50 text-emerald-800" : "bg-slate-100 text-muted")}>{account.active ? common.active : common.inactive}</span>
                ...
                <form action={setAccountActive}>
                  ...
                  <button className="min-h-11 rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-black text-ink shadow-card transition hover:border-primary/40 hover:text-primary">{account.active ? common.deactivate : common.activate}</button>
                </form>
              </div>

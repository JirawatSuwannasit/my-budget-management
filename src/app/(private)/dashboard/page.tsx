...
export default async function DashboardPage() {
  ...
  try {
    ...
    if (hasRealDashboardRows(rows)) {
      ...
      source = "supabase";
      status = "ready";
      notices = ["Loaded from your private Supabase tables."];
    } else {
      status = "empty";
      notices = ["No finance records found yet. Showing clearly labeled demo data until you add accounts and transactions."];
    }
  } catch (error) {
    status = "error";
    errorMessage = error instanceof Error ? error.message : "Unable to load dashboard data.";
    notices = ["Could not load Supabase dashboard data. Showing clearly labeled demo data so the layout remains reviewable."];
  }

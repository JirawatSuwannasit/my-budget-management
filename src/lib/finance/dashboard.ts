...
export function calculateDashboardSnapshot(input: DashboardInput): DashboardSnapshot {
  ...
  const remainingCreditCardPayable = sum(input.creditCards.map((card) => card.billedOutstanding));
  const currentCardCycleSpending = sum(input.creditCards.map((card) => card.currentCycleSpending));
